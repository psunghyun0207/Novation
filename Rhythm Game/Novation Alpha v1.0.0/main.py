...
while IsRunning:
    ...
    if Window == 'Select_Song' and WindowPlay == 1:
        ...
        Font_Level_Easy_Size = Font_Level_Easy.get_size()
        Font_Level_Easy_Horizon = Font_Level_Easy_Size[0]
        Font_Level_Easy_Vertical = Font_Level_Easy_Size[1]
        
        Font_Level_Normal_Size = Font_Level_Normal.get_size()
        Font_Level_Normal_Horizon = Font_Level_Normal_Size[0]
        Font_Level_Normal_Vertical = Font_Level_Normal_Size[1]
        
        Font_Level_Hard_Size = Font_Level_Hard.get_size()
        Font_Level_Hard_Horizon = Font_Level_Hard_Size[0]
        Font_Level_Hard_Vertical = Font_Level_Hard_Size[1]
        ...
        if(Difficult == 0):
            pygame.draw.rect(Screen, GREEN, (w/16, h - h/7, w/4, h/8), 0)
            pygame.draw.rect(Screen, BLUE, (w/2 - w/8, h - h/7, w/4, h/8), 5)
            pygame.draw.rect(Screen, RED, (w - w/4 - w/16, h - h/7, w/4, h/8), 5)
        elif(Difficult == 1):
            pygame.draw.rect(Screen, GREEN, (w/16, h - h/7, w/4, h/8), 5)
            pygame.draw.rect(Screen, BLUE, (w/2 - w/8, h - h/7, w/4, h/8), 0)
            pygame.draw.rect(Screen, RED, (w - w/4 - w/16, h - h/7, w/4, h/8), 5)
        elif(Difficult == 2):
            pygame.draw.rect(Screen, GREEN, (w/16, h - h/7, w/4, h/8), 5)
            pygame.draw.rect(Screen, BLUE, (w/2 - w/8, h - h/7, w/4, h/8), 5)
            pygame.draw.rect(Screen, RED, (w - w/4 - w/16, h - h/7, w/4, h/8), 0)
        
        Screen.blit(Font_Level_Easy, (w/16 + w/64, h - h/7 + h/32))
        Screen.blit(Font_Level_Normal, (w/2 - w/8 + w/64, h - h/7 + h/32))
        Screen.blit(Font_Level_Hard, (w - w/4 - w/16 + w/64, h - h/7 + h/32))
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    ...
                    while True:
                        Word = Current_Play_Song_Pattern.readline()
                        print(Word, Find)
                        if Word == Find:
                            break
